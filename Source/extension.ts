...
const getWorkspaceFolderForTestFile = (uri: vscode.Uri) =>
	(uri.path.endsWith(".test.ts") ||
		uri.path.endsWith(".integrationTest.ts")) &&
...
export async function activate(context: vscode.ExtensionContext) {
		...
		return async (
			...
			const listener = fileChangedEmitter.event(({ uri, removed }) => {
				...
				debounced = setTimeout(() => {
					const include =
						req.include?.filter(
							(t) => t.uri && queuedFiles.has(t.uri?.toString()),
						) ??
						[...queuedFiles]
							.map((f) =>
	...
	for (const [name, arg] of browserArgs) {
		...
		cfg.configureHandler = () =>
			vscode.window.showInformationMessage(`Configuring ${name}`);
