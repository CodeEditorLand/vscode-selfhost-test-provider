	...
	constructor() {
		this.disposables.push(
			commands.registerCommand(
				Constants.FixCommandId,
				async (uri: Uri, position: Position) => {
					const document = await workspace.openTextDocument(uri);

					const failingAssertion =
						detectFailingDeepStrictEqualAssertion(
							document,
							position,
						);
...
const formatJsonValue = (value: unknown) => {
	...
	return tsPrinter.printNode(
		ts.EmitHint.Expression,
		unquoted.transformed[0],
		src,
	);
};
...
const assertionFailureMessageRe =
...
class StrictEqualAssertion {
	...
	public static atPosition(
		sf: ts.SourceFile,
		offset: number,
	): StrictEqualAssertion | undefined {
...
function getAllTestStatusMessagesAt(
	uri: Uri,
	lineNumber: number,
): TestMessage[] {
...
function getTestResultsWithUri(
