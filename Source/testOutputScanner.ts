...
type QueuedOutput =
	| string
	| [string, vscode.Location | undefined, vscode.TestItem | undefined];
	...
	try {
		...
		await new Promise<void>((resolve) => {
			...
			scanner.onOtherOutput((str) => {
				...
				const logLocation = store.getSourceLocation(
					match[2],
					Number(match[3]),
				);
				...
				enqueueOutput(() =>
					Promise.all([logLocation, logContents]).then(
						([location, contents]) => [
							contents + crlf,
							location,
							test,
		...
		if (sm.sourceRoot) {
			try {
				return vscode.Uri.parse(
					new URL(source, sm.sourceRoot).toString(),
				);
			} catch {
				// ignored
			}
