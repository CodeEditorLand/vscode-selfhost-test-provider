...
export abstract class VSCodeTestRunner {
	...
	public async debug(
		...
		if (!baseConfiguration) {
			throw new Error(
				`Could not find launch configuration ${ATTACH_CONFIG_NAME}`,
			);
		}
			...
			DEBUG_TYPE,
			{
				createDebugAdapterTracker(session) {
					if (
						...
						session.parentSession !== rootSession
					) {
						return;
					}
					...
					return {
						onDidSendMessage(message) {
							if (
								...
								message.request_seq === initRequestId
							) {
								server.ready();
							}
						},
		...
		vscode.debug.startDebugging(this.repoLocation, {
			...baseConfiguration,
			port,
		});
	...
	private prepareArguments(
		...
		const addTestFileRunPath = (data: TestFile) =>
			runPaths.add(
				path
					.relative(data.workspaceFolder.uri.fsPath, data.uri.fsPath)
					.replace(/\\/g, "/"),
			);
	...
	protected async readProductJson() {
		...
		try {
			...
			throw new Error(
				`Error parsing product.json: ${(e as Error).message}`,
			);
		}
	}
